...
def draw_detections(img, out_dir, data_name, img_id, intrinsics, pred_sRT, pred_size, pred_class_ids,
    ...
    """
    out_path = os.path.join(out_dir, '{}_{}_bottle_cgi.png'.format(data_name, img_id))
    # import cv2
    # img = cv2.imread('/home/yliu/Desktop/project/Learning/experience/IST-Net-30/log/visual/1228/real_1228_can_cgi.png', cv2.IMREAD_UNCHANGED)
    # img = np.array(img)
    ...
    j = 1
    if gt_class_ids[j] in [1, 2, 4]:
        sRT = align_rotation(gt_sRT[j, :, :])
    else:
        sRT = gt_sRT[j, :, :]
    bbox_3d = get_3d_bbox(gt_size[j, :], 0)
    transformed_bbox_3d = transform_coordinates_3d(bbox_3d, sRT)
    projected_bbox2 = calculate_2d_projections(transformed_bbox_3d, intrinsics)
    img = draw_bboxes(img, projected_bbox2, (0, 255, 0))
    i = 1
    if pred_class_ids[i] in [1, 2, 4]:
        sRT = align_rotation(pred_sRT[i, :, :])
    else:
