...
from utils.scheduler import BNMomentumScheduler #, CyclicLR
from utils.evaluation_utils import compute_3d_matches_for_each_gt
from utils.vis_utils import draw_detections

from model.seg_model import seg_model
from model.ist_net import ShapeLoss
...
class Solver(gorilla.solver.BaseSolver):
    ...
    def step(self, syn_data, real_data, mode):
        ...
        pts = data['pts']
        pcl_data = {'points': pts}
        obj_id = data['category_label']
        class_id = obj_id.cpu().numpy() + 1
        ...
        # segmentation model
        classid = np.stack([np.array(onehot_dict[num]) for num in class_id.flatten()])
        point_seg_result, target_features, all_feature, center_points, labels = seg_model(pcl_data, classid)  # [8, 1024, 257]

        end_points = self.model(data, all_feature)
    ...
    def geo_loss_point(self, points, p_R, p_t, g_R, g_t, sym):

        bs = points.shape[0]
        points_re = torch.bmm(g_R.permute(0, 2, 1), (points - g_t.view(bs, 1, -1)).permute(0, 2, 1))

        points_re_p = torch.bmm(p_R.permute(0, 2, 1), (points - p_t.view(bs, 1, -1)).permute(0, 2, 1))
        res_geo = self.loss_func(points_re_p, points_re)
    ...
    with tqdm(total=len(dataloder)) as t:
        for i, data in enumerate(dataloder):
            ...
            if i == 352:
                ...
                pts = data['pts']
                if len(pts.squeeze().size()) == 2:
                    continue
                pcl_data = {'points': pts.squeeze()}
                obj_id = data['category_label']
                class_id = obj_id.cpu().numpy() + 1
                ...
                onehot_dict = {1: [1, 0, 0, 0, 0, 0],
                               ...
                               6: [0, 0, 0, 0, 0, 1]}
                classid = np.stack([np.array(onehot_dict[num]) for num in class_id.flatten()])
                # point_seg_result = seg_model(pcl_data, classid)  # [8, 1024, 257]

                point_seg_result, target_features, all_feature, center_points, labels = seg_model(pcl_data, classid)
                end_points = model(inputs, point_seg_result, all_feature, labels)
                pred_c = end_points['pred_confidence']
                sorting_indices = torch.argsort(pred_c, dim=1, descending=True)
                max_sorting_indices = sorting_indices[:, 0].squeeze(1)

                translation = end_points['pred_translation']
                size = end_points['pred_size']
                rotation = end_points['pred_rotation']
                bs = rotation.size(0)
                pred_translation = torch.zeros(bs, 3)
                pred_size = torch.zeros(bs, 3)
                pred_rotation = torch.zeros(bs, 3, 3)
                for j in range(bs):
                    pred_translation[j] = translation[j, max_sorting_indices[j]]
                    pred_size[j] = size[j, max_sorting_indices[j]]
                    pred_rotation[j] = rotation[j, max_sorting_indices[j]]
                pred_scale = torch.norm(pred_size, dim=1, keepdim=True)
                pred_size = pred_size / pred_scale

                num_instance = pred_rotation.size(0)
                pred_RTs = torch.eye(4).unsqueeze(0).repeat(num_instance, 1, 1).float()
                pred_RTs = pred_RTs.to(pred_rotation.device)
