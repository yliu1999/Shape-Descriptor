...
class get_model(nn.Module):
    ...
    def forward(self, 
                vertices: "tensor (bs, vetice_num, 3)",  # pcl
                onehot: "tensor (bs, cat_num)"):  # label
        """
        ...
        neighbor_index = gcn3d.get_neighbor_index(vertices, self.neighbor_num)  # [bs, 1024, 50]
        ...
        onehot = onehot.squeeze().unsqueeze(1).repeat(1, vertice_num, 1) #(bs, vertice_num, cat_one_hot)  [1, 8, 6]->[8, 1024, 6]
...
class get_loss(torch.nn.Module):
    ...
    def forward(self, pred, target, cate_sym, category):  # pred(pred label): [bs, 1024, 257], target(target label):[bs, 6, 1024]
        # print(pred.shape, target.shape)
        nsym_batch = (cate_sym == 0)
        nsym_pred = pred[nsym_batch]  # [nsym_bs, 1024, 257]
        nsym_target = target[nsym_batch] # [nsym_bs, 6, 1024]
        ...
        if(nsym_pred.shape[0]):
            ...
            nsym_pred = nsym_pred.contiguous().view(-1, nsym_pred.shape[-1])  # [nsym_bs*1024, 257]
            nsym_target = nsym_target[:,0,:].contiguous().view(-1)  # [nsym_bs*1024,]
            nsym_loss = F.nll_loss(nsym_pred, nsym_target, reduction='none').view(bs,-1)  # [nsym_bs, 1024]  # 用于计算负对数似然损失（negative log-likelihood loss）。它通常在多分类任务中用作损失函数
            nsym_loss = torch.mean(nsym_loss, dim=-1)  # [nsym_bs,]
            loss = torch.cat((loss, nsym_loss), dim=-1)
        if(sym_pred.shape[0]):
            bs = sym_pred.shape[0]
            sym_pred = sym_pred.unsqueeze(1).repeat(1,sym,1,1).view(-1,sym_pred.shape[-1])  # [sym_bs*6*1024, 257]
            sym_loss = F.nll_loss(sym_pred, sym_target.view(-1), reduction='none')  # [sym_bs#6#1024]
            # print(loss.shape)
            sym_loss = sym_loss.view(bs,sym,-1)  # [sym_bs, 6, 1024]
            sym_loss = torch.mean(sym_loss, dim=-1)  # [sym_bs, 6]
            # print(loss.shape)
            sym_loss = torch.min(sym_loss, dim=-1)[0]  # [sym_bs]
