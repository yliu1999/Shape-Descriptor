...
from provider.data_augmentation import data_augment, get_rotation

from utils.data_utils import load_depth, load_composed_depth, get_bbox, fill_missing

from utils.common_utils import write_obj
...
class TestDataset():
    def __init__(self, config, data_dir):
        ...
        result_pkl_list = glob.glob(os.path.join(self.data_dir, 'NOCS', 'segmentation_results', 'test_trainedwithMask', 'results_*.pkl'))
        ...
        self.class_name_map = {1: 'bottle_',
                               ...
                               6: 'mug_'}
        self.models = {}
        with open(os.path.join(self.data_dir, 'NOCS', model_path), 'rb') as f:
            self.models.update(cPickle.load(f))
    ...
    def __getitem__(self, index):
        ...
        image_path = image_path.replace('/data/real/', '/NOCS/Real/')
